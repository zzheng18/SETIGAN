...
for i, (imgs, labels) in enumerate(tqdm(dataloader)):
    ...
    if (result_np>0.598).all() & (result_np<0.6).all():
...
for i, (imgs, labels) in enumerate(tqdm(dataloader)):
    ...
    result = discriminator(imgs, labels)
    result_np = result.cpu().detach().numpy()
    if (result_np>0.27).all() & (result_np<0.275).all():
        loc = 'orange_red_o/'+str(i)+'.png'
        save_image(imgs.data.reshape(1,1,128,128), loc, nrow=1, normalize=True)
    if (result_np>0.07).all() & (result_np<0.073).all():
...
for i, (imgs, labels) in enumerate(tqdm(dataloader)):
    ...
    result = discriminator(imgs, labels)
    result_np = result.cpu().detach().numpy()
    if (result_np>0.475).all() & (result_np<0.48).all():
        loc = 'red_blue_b/'+str(i)+'.png'
        save_image(imgs.data.reshape(1,1,128,128), loc, nrow=1, normalize=True)
    if (result_np>0.59).all() & (result_np<0.593).all():
...
for i, (imgs, labels) in enumerate(tqdm(dataloader)):
    ...
    result_np = result.cpu().detach().numpy()
    if (result_np>0.27).all() & (result_np<0.275).all():
        loc = 'orange_red_r/'+str(i)+'.png'
        save_image(imgs.data.reshape(1,1,128,128), loc, nrow=1, normalize=True)
    if (result_np>0.475).all() & (result_np<0.48).all():
        loc = 'red_blue_r/'+str(i)+'.png'
        save_image(imgs.data.reshape(1,1,128,128), loc, nrow=1, normalize=True)
    if (result_np>0.415).all() & (result_np<0.418).all():
